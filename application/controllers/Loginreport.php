...
class Loginreport extends CI_Controller {
    ...
    function calculate_distace_travel($date, $user, $uniq_login_id = null){
		$data['date'] 		=	$date;
		$data['user_id'] 	= 	$user;
		$data['uniqLoginId'] = $uniq_login_id;
		...
		echo '[';
			...
			for($i=0;$i<$c;$i++){
				...
				$data = array(
					...
					'lgnrKmTravel'		=>	$this->calculate_distace_travel($lgnrlist[$i]['lgnrLoginDate'],$lgnrlist[$i]['lgnrUserId'], $lgnrlist[$i]['uniq_login_id']),
					'uniq_login_id' => $lgnrlist[$i]['uniq_login_id'],
